...
export default function TransfersPage() {
  ...
  function getStatus(transfer: TransferRecord) {
  ...
  const departments = useMemo(() => {
    const values = transfers
      .map((transfer) => transfer.department)
      .filter(
  ...
  const filteredTransfers = useMemo(() => {
    ...
    return transfers.filter((transfer) => {
      const equipment = Array.isArray(transfer.assets)
        ? transfer.assets[0]
        : transfer.assets;
      ...
      const transferStatus = getStatus(transfer);
      ...
      const matchesDepartment =
        ...
        transfer.department === departmentFilter;
  ...
  const activeCount = transfers.filter(
    (transfer) => !transfer.returned_date
  ).length;

  const overdueCount = transfers.filter(
    (transfer) =>
      !transfer.returned_date &&
      transfer.due_date < today
  ).length;

  const dueTodayCount = transfers.filter(
    (transfer) =>
      !transfer.returned_date &&
      transfer.due_date === today
  ).length;

  const returnedCount = transfers.filter(
    (transfer) => transfer.returned_date !== null
  ).length;

  async function returnEquipment(transfer: TransferRecord) {
    const equipment = Array.isArray(transfer.assets)
      ? transfer.assets[0]
      : transfer.assets;
    ...
    setReturningTransferId(transfer.id);
    setErrorMessage("");
    ...
    if (transferError) {
      console.error("Transfer return error:", transferError);
      ...
      <div className="mt-8 rounded-2xl bg-white p-8 shadow">
        ...
        {loading ? (
          ...
          <TransfersTable
            ...
            returningTransferId={returningTransferId}
